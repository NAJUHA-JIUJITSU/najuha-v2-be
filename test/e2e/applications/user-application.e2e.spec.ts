...
describe('E2E u-6 applications TEST', () => {
  ...
  describe('u-6-10 cancelApplicationOrder', () => {
    it('결제 주문 취소 성공', async () => {
      ...
      const partialCanceledApplication = cancelApplicationOrderResBody.result.application;

      expect(partialCanceledApplication.status).toBe('PARTIAL_CANCELED');
      const partialCanceledApplicationOrder = partialCanceledApplication.applicationOrders?.find(
        (order) => order.orderId === orderId,
      );
      if (!partialCanceledApplicationOrder)
        throw new Error('partialCanceledApplicationOrder is not exist in applicationOrders');
      expect(partialCanceledApplicationOrder.status).toBe('PARTIAL_CANCELED');
      expect(partialCanceledApplicationOrder.applicationOrderPaymentSnapshots.length).toBe(2);
      expect(
        partialCanceledApplicationOrder.applicationOrderPaymentSnapshots[1].participationDivisionInfoPayments.length,
      ).toBe(3);
      expect(
        partialCanceledApplicationOrder.applicationOrderPaymentSnapshots[1].participationDivisionInfoPayments.forEach(
          (participationDivisionInfoPayment) => {
            expect(participationDivisionInfoPayment.status).toBe('DONE');
          },
        ),
